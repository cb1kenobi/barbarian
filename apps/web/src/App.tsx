...
export function App() {
  ...
  const load = useCallback(async () => {
    try { setDashboard(await api<Dashboard>('/api/dashboard')); setError(null); }
    ...
    <div className="shell">
      <aside className="rail">
        ...
        <div className="rail-bottom">
          ...
          <button className="settings-link" onClick={() => setShowSettings(true)}>⚙ Settings</button>
        </div>
      </aside>
      ...
      <main id="command">
        ...
        <section id="work" className="panel">
          <div className="panel-head"><div><span className="section-label">YOUR WORK</span><h2>Priority queue</h2></div><span className="panel-note">Excludes duplicates, fixed work, and claimed issues</span></div>
          <div className="rows">
        ...
        <section id="reviews" className="panel reviews">
          <div className="panel-head"><div><span className="section-label">CODE REVIEWS</span><div className="review-heading"><h2>Review queue</h2><span className="review-count" aria-label={`${reviewsNeedingApproval} non-approved pull requests need your review`}>{reviewsNeedingApproval} to review</span><ReviewStatusInfo /></div></div><label className="review-sort"><span>Sort</span><select value={reviewSort} onChange={(event) => setReviewSort(event.target.value as ReviewSort)}><option value="priority">Priority</option><option value="oldest">Oldest</option><option value="newest">Newest</option><option value="repository">Repo name</option></select></label></div>
