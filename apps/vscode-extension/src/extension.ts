...
<script nonce="${nonce}">
const vscode=acquireVsCodeApi();let context;let selection;let busy=false;let hideResolved=false;
...
function scrollTranscriptToBottom(){const transcript=main.querySelector('.transcript');if(transcript)requestAnimationFrame(()=>{transcript.scrollTop=transcript.scrollHeight})}
...
window.addEventListener('message',({data})=>{if(data.type==='loading'&&!context)main.innerHTML='<p class="loading">Reading the current branch…</p>';if(data.type==='context'){const previousMessageCount=context?.messages?.length||0;context=data.context;applyAppearance(context?.appearance);busy=false;render(Boolean(data.changedBranch),(context.messages?.length||0)>previousMessageCount)}if(data.type==='selection'){selection=data.selection;updateSelection()}if(data.type==='busy'){busy=true;document.querySelectorAll('button').forEach(b=>b.disabled=true);const status=main.querySelector('.action-status')||main.querySelector('.error');if(status)status.textContent=data.message}if(data.type==='asked'&&context){context.messages=[...(context.messages||[]),{id:'pending-'+Date.now(),role:'user',author:'VS Code extension',content:data.message}];render(true,true)}if(data.type==='error'){busy=false;document.querySelectorAll('button').forEach(b=>b.disabled=false);const error=main.querySelector('.error')||main.querySelector('.action-status');if(error)error.textContent=data.message}if(data.type==='offline'){context=undefined;busy=false;key.textContent='Branch review';main.innerHTML='<p class="offline"><strong>Branch context unavailable</strong>'+escapeHtml(data.message)+'</p><p class="empty">Fix the error above, then refresh this view.</p>'}});
</script></body></html>`;
  }
}
