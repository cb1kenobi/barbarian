...
function restoreTranscriptScroll(snapshot){const transcript=main.querySelector('.transcript');if(!transcript)return;let anchorDelta=0;if(snapshot&&!snapshot.pinned){const messages=Array.from(transcript.querySelectorAll('[data-message-id]')),anchor=snapshot.anchors?.find(candidate=>messages.some(message=>message.dataset.messageId===candidate.id)),message=anchor&&messages.find(candidate=>candidate.dataset.messageId===anchor.id);if(message)anchorDelta=message.getBoundingClientRect().top-transcript.getBoundingClientRect().top-anchor.top}transcript.scrollTop=!snapshot||snapshot.pinned?transcript.scrollHeight:Math.min(Math.max(0,snapshot.scrollTop+anchorDelta),Math.max(0,transcript.scrollHeight-transcript.clientHeight))}
const renderContent=render;
render=function(resetDraft=false){const nextConversationKey=context?.review?.id||context?.branch?.id||'';const snapshot=nextConversationKey===renderedConversationKey?captureTranscriptScroll():undefined;renderContent(resetDraft,false);renderedConversationKey=nextConversationKey;restoreTranscriptScroll(snapshot)};
