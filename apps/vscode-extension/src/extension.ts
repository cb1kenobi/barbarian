...
function captureTranscriptScroll(){const transcript=main.querySelector('.transcript');if(!transcript)return;return{scrollTop:transcript.scrollTop,pinned:transcript.scrollHeight<=transcript.clientHeight||transcript.scrollHeight-transcript.clientHeight-transcript.scrollTop<=4}}
function restoreTranscriptScroll(snapshot){const transcript=main.querySelector('.transcript');if(!transcript)return;transcript.scrollTop=!snapshot||snapshot.pinned?transcript.scrollHeight:Math.min(snapshot.scrollTop,Math.max(0,transcript.scrollHeight-transcript.clientHeight))}
