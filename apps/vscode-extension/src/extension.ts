...
<script nonce="${nonce}">
const vscode=acquireVsCodeApi();let context;let selection;let busy=false;let chatPending=false;let hideResolved=false;let renderedConversationKey='';
...
window.addEventListener('message',({data})=>{if(data.type==='loading'&&!context)main.innerHTML='<p class="loading">Reading the current branch…</p>';if(data.type==='context'){context=data.context;applyAppearance(context?.appearance);if(!chatPending)busy=false;render(Boolean(data.changedBranch))}if(data.type==='selection'){selection=data.selection;updateSelection()}if(data.type==='busy'){busy=true;document.querySelectorAll('button').forEach(b=>b.disabled=true);const textarea=main.querySelector('textarea');if(textarea)textarea.disabled=true;const status=main.querySelector('.action-status')||main.querySelector('.error');if(data.message&&status)status.textContent=data.message}if(data.type==='asked'&&context){chatPending=true;context.messages=[...(context.messages||[]),{id:'pending-'+Date.now(),role:'user',author:'VS Code extension',content:data.message}];render(true)}if(data.type==='chatPending'){chatPending=Boolean(data.pending);if(!chatPending)busy=false;render()}if(data.type==='restoreDraft'){const textarea=main.querySelector('textarea');if(textarea&&!textarea.value){textarea.value=data.message||'';textarea.focus()}}if(data.type==='error'){chatPending=false;busy=false;render();document.querySelectorAll('button').forEach(b=>b.disabled=false);const error=main.querySelector('.error')||main.querySelector('.action-status');if(error)error.textContent=data.message}if(data.type==='offline'){context=undefined;busy=false;chatPending=false;renderedConversationKey='';key.textContent='Branch review';main.innerHTML='<p class="offline"><strong>Branch context unavailable</strong>'+escapeHtml(data.message)+'</p><p class="empty">Fix the error above, then refresh this view.</p>'}});
</script></body></html>`;
  }
}
