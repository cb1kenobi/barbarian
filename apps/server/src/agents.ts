...
export function hasPublishedCleanReview(bundle: ReviewBundle, headSha: string): boolean {
  const reviews = Array.isArray(bundle.metadata.reviews) ? bundle.metadata.reviews : [];
  const shortSha = headSha.slice(0, 8).toLowerCase();
  const isCleanBody = (body: unknown) => typeof body === 'string'
    && /generated by barber ai/i.test(body)
    && (() => {
      const normalized = body.trim().toLowerCase();
      return normalized.startsWith(`reviewed ${shortSha}`) || normalized.startsWith(`barbarian reviewed ${shortSha}`);
    })();
  const submittedReviewExists = reviews.some((value) => {
    if (!value || typeof value !== 'object') return false;
    const review = value as { body?: unknown; commit?: { oid?: unknown } | null };
    return review.commit?.oid === headSha && isCleanBody(review.body);
  });
  if (submittedReviewExists) return true;
  return bundle.issueComments.some((value) => value && typeof value === 'object'
    && isCleanBody((value as { body?: unknown }).body));
}
...
${JSON.stringify(bundle)}`;
    ...
    const publishCleanReview = result.findings === 0
      && claim.trigger !== 'feedback'
      && !hasPublishedCleanReview(bundle, claim.headSha);
    if (signal?.aborted) throw signal.reason || new Error('Review stopped');
    if (commentsToPublish.length > 0 || publishCleanReview) {
    ...
    recordActivity(
      ...
      claim.reviewId,
      {
        ...
        publishedCleanReview: publishCleanReview,
