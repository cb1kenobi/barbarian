...
export async function resolveReviewWorkspace(
  ...
  const branch = database.connection.prepare(`
    SELECT id, repository, branch_name, workspace_path
    FROM local_branches WHERE review_id=?
  ...
  const [currentBranch, origin] = await Promise.all([
    runProcess('git', ['branch', '--show-current'], {
      cwd: resolvedPath, timeoutMs: 20_000, maxOutputCharacters: 2_000,
    }),
