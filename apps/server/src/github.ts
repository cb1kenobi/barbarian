...
export async function postPullRequestReview(
  ...
  const publication = reviewPublication(repository, number, headSha, summary, comments);
...
export function cleanReviewCommentBody(headSha: string, summary: string): string {
  const signature = '—\n_Generated by Barber AI_';
  return `Reviewed ${headSha.slice(0, 8)} and found no blocking issues. ${summary}\n\n${signature}`.trim();
}

export function reviewPublicationPayload(headSha: string, comments: ReviewCommentDraft[]) {
...
export function reviewPublication(
  ...
  if (comments.length > 0) {
    return {
      ...
      payload: reviewPublicationPayload(headSha, comments),
    };
  }
  return {
    endpoint: `repos/${repository}/issues/${number}/comments`,
    payload: { body: cleanReviewCommentBody(headSha, summary) },
  };
}
