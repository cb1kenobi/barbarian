...
describe('reviewPublicationPayload', () => {
  it('uses anonymous review attribution by default', () => {
    const payload = reviewPublicationPayload('1234567890', 'One issue was found.', [
      { path: 'file.ts', line: 2, side: 'RIGHT', body: '**High: broken invariant**\n\nFailure mode.' },
    ]);
    expect(payload.body).toContain('One issue was found.\n\n—\nReviewed 12345678');
  ...
  it('uses the configured review name and replaces agent-supplied attribution', () => {
    const payload = reviewPublicationPayload('abcdef123456', 'One issue.', [
      {
  ...
  it('publishes clean results as a visible GitHub review', () => {
    const publication = reviewPublication('Acme/repo', 12, '1234567890', 'The change is safe.', []);
    expect(publication.payload).toEqual({
      commit_id: '1234567890', event: 'COMMENT',
      body: 'The change is safe.\n\n—\nReviewed 12345678', comments: [],
    });
  });
  ...
  it('cannot accidentally publish a review without a summary', () => {
