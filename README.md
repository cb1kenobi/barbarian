...
Add zero or more code review rows in Settings, with an independent provider, model, effort, and priority. One available row handles each review. Random and round-robin routing spread work across the pool; priority routing chooses the highest number first. Before every selection and failover, providers with known usage above `100 - usageHeadroomPercent` are removed. Codex and token-authenticated Claude providers have best-effort built-in usage checks; a failed check remains eligible as “usage unknown” and its error is shown in the review dropdown. Other CLIs remain eligible when their usage is unknown; provider definitions can supply `usageCommand: [command, arg]`, whose stdout must be a percentage or `{"usedPercent": 42}`.
